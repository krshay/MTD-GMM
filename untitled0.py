# -*- coding: utf-8 -*-
"""
Created on Wed Sep  8 10:43:50 2021
...
def TryPar(it, xs, gamma, L, N, SNRs, shifts_2nd, shifts_3rd, x_gamma0s,Nguesses, NSNRs):
    print(f'iter #{it}')
    
    
    results_mom = np.zeros((NSNRs, Nguesses), dtype=scipy.optimize.optimize.OptimizeResult)
    results_gmm = np.zeros((NSNRs, Nguesses), dtype=scipy.optimize.optimize.OptimizeResult)
    times_mom = np.zeros((NSNRs, Nguesses))
    times_gmm = np.zeros((NSNRs, Nguesses))
    ...
    for (idx, SNR) in enumerate(SNRs):
       ...
       # del y
        ...
        for i in range(Nguesses):
            ...
            times_mom[idx, i] = time.time() - start
            ...
            times_gmm[idx, i] = time.time() - start
            estimations_gmm.append(estimation_gmm)
            print(estimation_gmm.fun)
        #del samples
        #del f_gmm
        results_mom[idx, :] = estimations_mom
        results_gmm[idx, :] = estimations_gmm
...
results = pool.starmap(TryPar, [(it, xs, gamma, L, N, SNRs, shifts_2nd, shifts_3rd, x_gamma0s,Nguesses, NSNRs) for  it in range(Niters)])
