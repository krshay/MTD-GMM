...
Importantly, under the well-separated model, for~\mbox{$\ell$-s} in the range~$\mathcal{L} = \{0, \ldots, {L - 1}\}$, any given occurrence of~$x$ in~$y$ is only ever correlated with itself, and never with another occurrence.
