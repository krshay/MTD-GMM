...
%
\begin{abstract}
We consider the one-dimensional multi-target detection (MTD) problem of recovering a target signal from a noisy measurement that contains multiple copies of the signal, each randomly translated\asafc{is translated is the right word?}. Motivated by the structure reconstruction problem in single-particle cryo-electron microscopy, we focus on the high noise regime, where the noise hampers accurate detection of the signal occurrences\asafc{I don't think this the last sentance is a good desripition of the work. A more fitting one: ''The MTD problem is a simplified model of the tructure reconstruction problem in single-particle cryo-electron microscopy (cryo-EM), this work chief motivation.''}. We develop a generalized method of moments (GMM) framework to estimate the image directly from a measurement, and compare the framework to the method of moments through numerical experiments. We demonstrate signal recovery in highly noisy measurements, and prove numerically the optimality of the generalized method of moments.
\end{abstract}
%
\begin{keywords}
...
Importantly, under the well-separated case, %for~\mbox{$\ell$-s} in the range~$\mathcal{L} = \{0, \ldots, {L - 1}\}$,
...
Following~\cite{bendory2019multi}, \cite{lan2020multi}, \cite{marshall2020image}, \cite{bendory2021multi}, \cite{kreymer2021two}\asafc{I feel that here we only should cite only works that do LS in 1D.} and based on the notations presented above, we formulate a non-convex least squares problem for estimating the signal~$x$ from the autocorrelations of the measurement~$y$:
\begin{align}
