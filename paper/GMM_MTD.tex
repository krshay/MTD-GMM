...
% Title.
% ------
\title{The generalized method of moments for multi-target detection}
%
% Single address.
% ---------------
...
Autocorrelation analysis is a special case of the method of moment, which is a classical statistical inference technique, tracing back to 1894~\cite{pearson1894contributions}. This work studies the application of the generalized method of moments, or, more precisely, the \textit{generalized autocorrelation analysis}, to the MTD problem~\cite{Hansen1982}. In the classical autocorrelation analysis, we wish to find a signal whose autocorrelations match the observed autocorrelations. This is usually done by minimizing a LS objective. The generalized autocorrelation analysis suggests replacing the LS objective with a weighted LS, and provides a recipe of how to compute the optimal weights; see Section~\ref{subsec:generalized_ac}. The generalized method of moments has been proven to be highly effective in a variety of computational task, such as~\cite{wooldridge2001applications,akbar2016more,fan2018optimal,roodman2009xtabond2, abas2021generalized}.
We also mention that the framework can be formulated with other objective functions, rather than LS~\cite{Jong2002}.
...
\begin{multline} \label{Eq-GMM-2}
	...
	\end{bmatrix},
\end{multline}
\asafc{I think it is unreadable}
...
\end{equation}
Here, $W_N$ is a fixed positive semi-definite (PSD) matrix. Note that the LS estimator~\eqref{eq:optimization} is a special case of~\eqref{Eq-2-1}, where~$W_N$ is the diagonal matrix with the weights from~\eqref{eq:optimization}
...
\label{sec:numerical}
This section compares the numerical performance of the generalized method of moments and the classical autocorrelation analysis. The optimization problem~(\ref{eq:optimization}) was minimized using the~\mbox{Broyden-Fletcher-Goldfarb-Shanno}~(BFGS) algorithm, while ignoring the positivity constraint on~$\gamma$ (namely, treating it as an unconstrained problem).  We measure the estimation error by
\begin{equation*}
