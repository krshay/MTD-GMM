...
%	\hfill
	...
	\begin{subfigure}[ht]{0.49\columnwidth}
		...
		\caption{$\text{SNR} = 0.1$.}
	\end{subfigure}
	\caption{Two measurements at different noise levels:~\mbox{$\text{SNR} = 50$} (left);~\mbox{$\text{SNR} = 0.1$} (right). Each measurement contains multiple copies of the target signal. Our goal is to estimate the target signal directly from~$y$.}
...
Autocorrelation analysis is a variation of the method of moment (MoM), which is a classical statistical inference technique, tracing back to 1894~\cite{pearson1894contributions}. This work studies the application of the \textit{generalized method of moments} (GMM)  application into the MTD problem. The GMM theory, which was first introduced in~\cite{Hansen1982}, shows that the GMM provides an optimal estimator, in comparison with other weighted LS objective function. Previous work shows that the GMM suggests a significant improvement, e.g., \cite{fan2018optimal}, \cite{roodman2009xtabond2}, and specifically in cryo-EM~\cite{abas2021generalized}.

The main contribution of this paper is in extending the MoM framework introduced in~\cite{bendory2019multi} by developing a GMM framework for the one-dimensional MTD problem. We devise an algorithm for the recovery of the target signal from a measurement, and demonstrate a successful reconstruction in noisy regimes~(see Section~\ref{sec:numerical}). Moreover, we conduct numerical experiments which demonstrate the algorithm superiority. It is thus a step towards efficiently estimating a molecular structure directly from a noisy \mbox{cryo-EM} micrograph~\cite{bendory2018toward}.
%Moreover, we corroborate the theory and present superior results for the GMM estimator in comparison with the MoM estimator for the MTD model. It is thus a significant step towards efficiently estimating a molecular structure directly from a noisy \mbox{cryo-EM} micrograph~\cite{bendory2018toward}.
...
We discuss the \mbox{well-separated} case of the MTD problem, which was studied in~\cite{bendory2019multi}. In this case, we assume that each occurrence of~$x$ in the measurement~$y$ is separated by at least a full signal length~$L$ from its neighbors. Specifically, we assume that
\begin{equation}
\label{eq:sep}
...
To compute the third-order autocorrelation~(\ref{eq:AM3}), we compute the product of~$y$ with its two shifts. Importantly, for~\mbox{$\ell$-s} in the range~$\mathcal{L} = \{0, \ldots, {L - 1}\}$, any given occurrence of~$x$ in~$y$ is only ever correlated with itself, and never with another occurrence.\asafc{This section can be simply to something like: ''Under this condition, the autocorrelation analysis is simplified. Due to that, any given occurrence of~$x$ in~$y$ is only ever correlated with itself, and never with another occurrence. ''}
...
\gamma = p \frac{L}{N}.
\end{equation*}
\asafc{I don't think that this equation worth ths space...}
...
\end{align}
Note, $\A_x^3$ and $\A_y^3$ are treated as vectors.
\subsection{Signal recovery from autocorrelations}
Following~\cite{bendory2019multi}, \cite{lan2020multi}, \cite{marshall2020image}, \cite{bendory2021multi}, \cite{kreymer2021two} and based on the notations presented above, we formulate a non-convex least squares problem for estimating the signal~$x$ and the density~$\gamma$ from the autocorrelations of the measurement~$y$:
\begin{align}
...
\end{equation*}
where~$x^*$ is the true signal, and~$x$ is the estimated signal. In all experiments, the measurements are generated according to~(\ref{eq:model}) with density~$\gamma = 0.2$. The target signals in the measurements are of length~\mbox{$L = 21$}. In addition, in each trail, we initiate the optimization from~$5$ random guesses and choose the one whose final objective function is minimal.  The code to reproduce all experiments is publicly available at~\url{https://github.com/krshay/MTD-GMM}.


\subsection{Recovery error as a function of the measurement length}
\label{subsec:exp_size}
Figure~\ref{fig:err_size_experiment} presents recovery error as a function of the measurement length, for a fixed noise level ($\text{SNR} = 50$). We preform~$20$ trails for each measurement length,. As expected from the law of large numbers, both estimators' errors  decay as~$N^{-1/2}$. In addition, Theorem~\ref{Thm-2-6} provides the optimality of the GMM estimator over the MoM estimator, and indeed the median error of the GMM estimator is smaller than the error of the MoM estimator for all sizes. \asafc{I'm not sure if it is relevant to talk about the optimality here}

\begin{figure}[!tb]
	\begin{subfigure}[ht]{\columnwidth}
		\centering
		\includegraphics[width=0.9\columnwidth]{figures/experiment_size_err.pdf}
	\end{subfigure}
...
Next, we compared the recovery errors as a function of SNR, the results are in Figure~\ref{fig:err_noise_experiment}. The signal length~$y$ is fixed, $N = 10^6$. We  calculate the \textbf{median} over \textbf{20} trials.
\begin{figure}[!tb]
	\begin{subfigure}[ht]{\columnwidth}
		\centering
		\includegraphics[width=0.90\columnwidth]{figures/experiment_SNR_err.pdf}
	\end{subfigure}
...
This work in part as an on going research to improve the reconstruction results of cryo-EM, specifically without the need to locate the particles. Alongside with~\cite{bendory2018toward,abas2021generalized, kreymer2021two}, this work suggests a mathematical framework and a tempting estimator. In future work, we intend to apply the GMM of the 2D model of the problem in order to accelerate and improve the initial reconstruction phase.
