...
\end{equation*}
where~$x^*$ is the true signal, and~$x$ is the estimated signal. In experiments~\ref{subsec:exp_size} and~\ref{subsec:exp_SNR}, the measurements were generated according to~(\ref{eq:model}) with density~\mbox{$\gamma = 0.2$}, and the target signals~$x$ are of length~\mbox{$L = 21$}. Each entry in the signals was generated by~$x[i] \overset{\text{i.i.d.}}{\sim} \Unif[0, 1)$, and the target signals were normalized such that~$\|x\|_2  =1$. We try to estimate the signal from~$5$ random initial guesses for~$x$ and from~\mbox{$\gamma_{\text{init}} = 0.18$}, and calculate the estimation error for the estimate whose final objective function is minimal. Figures~\ref{fig:err_size_experiment} and~\ref{fig:err_noise_experiment} present the median error over 50 trials. The code to reproduce all experiments is publicly available at~\url{https://github.com/krshay/MTD-GMM}.
