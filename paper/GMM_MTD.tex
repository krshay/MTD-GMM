...
% Single address.
% ---------------
\name{Asaf Abas, Shay Kreymer, Ye'Ela Shalit, Ran Weber\thanks{The first two authors contributed equally to this work. S.K. is supported in part by the Yitzhak and Chaya Weinstein Research Institute for Signal Processing.}}
...
\end{equation}
and the third-order autocorrelation~\mbox{$A_{y}^3: \mathbb{Z}y \times \mathbb{Z} \rightarrow \mathbb{R}$} by
\begin{equation}
\label{eq:AM3}
A_{M}^3 [\ell_1, \ell_2] := \frac{1}{N} \sum_{i \in \mathbb{Z}} y[i] y[i + \ell_1] y[i + \ell_2].
\end{equation}
