...
Figure~\ref{fig:Micrograph_shifts}(a) presents an example of a measurement obeying the separation condition~(\ref{eq:sep}). To compute the third-order autocorrelation~(\ref{eq:AM3}), we compute the product of~$y$ with its two shifts. As demonstrated in Figure~\ref{fig:Micrograph_shifts}(b), for~\mbox{$\ell$-s} in the range
\begin{equation}
\label{eq:set_L}
